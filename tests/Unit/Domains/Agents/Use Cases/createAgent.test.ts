...
describe('createAgent - Use Case', () => {
  ...
  it('component public API', async () => {
    ...
    const { error, data }: IComponentResponse<IAgent> = await createAgent(James, { repos: { Agent: repo } });
  ...
  it('duplicated ID must return an error', async () => {
    ...
    let agent2;
    {
      const { error, data }: IComponentResponse<IAgent> = await createAgent(Steve, { repos: { Agent: repo } });
      expect(error).toBeNull();
      expect(data).toHaveProperty('id');
      expect(data).toHaveProperty('name');
      expect(data).toHaveProperty('createdAt');
    ...
    }
    {
      ...
      const { error, data }: IComponentResponse<IAgent> = await createAgent(agent2, { repos: { Agent: repo } });
